...
public class PokemonScreenLAB9 extends Application {
	...
	public void start(Stage stage) throws Exception {
		...
		currentPosx = (gen.nextInt(W)/STEP_SIZE) * STEP_SIZE; // this should be a random position
		currentPosy = (gen.nextInt(H)/STEP_SIZE) * STEP_SIZE; // this should be a random position

		Group mapGroup = new Group();
		avatar.relocate(currentPosx, currentPosy);
		mapGroup.getChildren().add(avatar);
		
		pikaImage = new Image(pika);
		pikachu = new ImageView(pikaImage);
		pikachu.setFitHeight(STEP_SIZE + 10);
		pikachu.setFitWidth(STEP_SIZE + 10);
